class GlApp {
    ...
    Render() {
        ...
        for (let i = 0; i < this.scene.models.length; i ++) {
            ...
            //Selects the shader
            if(selected_shader != 'emissive') {
                selected_shader = selected_shader + '_' + this.scene.models[i].shader;
            }
            ...
            // Store the light positions in an array
            let light_colors = [];
            let light_positions = [];
            for(let index = 0; index < this.scene.light.point_lights.length && index < 10; index++) {
                let light = this.scene.light.point_lights[index];
                light_colors = light_colors.concat([light.color[0], light.color[1], light.color[2]]);
                light_positions = light_positions.concat([light.position[0], light.position[1], light.position[2]]);
            }
            this.gl.uniform3fv(this.shader[selected_shader].uniform['light_color[0]'], Float32Array.from(light_colors));
            this.gl.uniform3fv(this.shader[selected_shader].uniform['light_position[0]'], Float32Array.from(light_positions));

            // Stores the texture information
